...
In some of the code, the operator `%>%` will be seen.  The operator is provided by the package `magrittr` [@R-magrittr].  It is a pipe that forwards the result of the preceeding function to the suceeding function.  By default, the forwarded object is applied as the first parameter of the succeeding function.  Using the `.` placeholder moves the argument to a specific parameter location in the succeeding function call.  The pipe is used where it improves readability.  A series of functions interspersed with `%>%` should be viewed as pipeline or processing elements, with the output of one function the input to the next.
...
```{r}
vo2data <- choose.files(filters = excelfilter,
...
ggplot(data=fuel_long,
  ...
  ggtitle(paste("Model predicted crossover for subject",subject)) + 
  geom_vline(xintercept = crossover,
             color = "darkgrey") +
  geom_text(x=crossover, 
            y=5, 
            label=paste("HR bpm at \n50% fat (predicted) =",
                        crossover),
            color = "blue",
            show.legend = FALSE)
```
