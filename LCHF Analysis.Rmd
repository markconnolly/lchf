...
summarized <- rer[,c("TestNumber","RERHR",
                     "MaxRelVO2","MaxAbsVO2",
                     "BFpctBIA","SFBFpct",
                     "MoodB4_cm","MoodAfter_cm",
                     "FatBIA","FatSF")] %>% 
   group_by(TestNumber) %>%
   summarise_each(funs(min,mean,median,sd,var,max))

boxplot(rer$RERHR ~ rer$TestNumber)
...
rer2_long <- gather(rer2,measurement,obs,RERHR:FatSF)

ggplot(data=rer2_long, aes(x=TestNumber,
                           y=obs,
                           color=Subject,
                           group = Subject)) + 
  geom_point() + 
